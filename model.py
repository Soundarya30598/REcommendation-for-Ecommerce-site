...
class SentimentRecommender:
    ...
    def __init__(self):
        self.model = pickle.load(open(
            SentimentRecommender.ROOT_PATH + SentimentRecommender.MODEL_NAME, 'rb'))
        self.vectorizer = pd.read_pickle(
            SentimentRecommender.ROOT_PATH + SentimentRecommender.VECTORIZER)
        self.user_final_rating = pickle.load(open(
            SentimentRecommender.ROOT_PATH + SentimentRecommender.RECOMMENDER, 'rb'))
        self.data = pd.read_csv("C:/Users/sahit/Downloads/Capstone_Project/sample30.csv")
        self.cleaned_data = pickle.load(open(
            SentimentRecommender.ROOT_PATH + SentimentRecommender.CLEANED_DATA, 'rb'))
